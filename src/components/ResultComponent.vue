...
export default {
                ...
                if (sizecheck == true) {
                    if (el.option1.toUpperCase() == size) {
                        ...
                        switch(size){
                            ...
                            var countertwo = 1;
                            for (
                                let i = 0; i <= this.product.variants.length; i++
                            ) {
    ...
    console.log(size +" " +this.container.sizeIndex)
        ...
        changesize: function (trigger) {
            ...
            var $currone = this.$allSlides.filter(":visible"), //get the visible slide
                $nxtTargetOne = $currone[action](".fit-advisor-selected-size"); //get the next target based on the action.
            $nxtTargetOne.addClass("active");

            $currone
                .stop(true, true)
                .fadeIn($time)
                .removeClass("active")
                .hide(); //hide curroneent one

            if (!$nxtTargetOne.length) {
                //if no next
                $time = 1;
                ...
                if (trigger == 0) {
                    ...
                    $nxtTargetOne = this.$allSlides["first"]();

                } else {



                    $nxtTargetOne = this.$allSlides["last"](); //based on traverse pick the next one
            ...
            $nxtTargetOne.stop(true, true).fadeIn($time).addClass("active"); //show the target
